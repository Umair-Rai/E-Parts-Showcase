...
const CustomerManagement = () => {
  ...
  useEffect(() => {
    if (!showModal && !showDeleteModal) return;
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        ...
        const response = await axios.get(buildApiUrl('/api/customers'), {
          headers: {
  ...
  const handleDeleteCustomer = async (customerId) => {
    try {
      ...
      await axios.delete(buildApiUrl(`/api/customers/${customerId}`), {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
    ...
    try {
      ...
      await Promise.all(
        selectedCustomers.map(customerId => 
          axios.delete(buildApiUrl(`/api/customers/${customerId}`), {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 w-full input-search admin-input"
                />
              </div>
            </div>
