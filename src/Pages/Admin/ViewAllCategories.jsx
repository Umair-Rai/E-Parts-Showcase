...
const ViewAllCategories = () => {
  ...
  const resolveCategoryImageUrl = (imagePath) => {
    if (!imagePath) return '';
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      const adminToken = localStorage.getItem('adminToken');
      await axios.delete(buildApiUrl(`/api/categories/${categoryToDelete}`), {
        headers: {
          Authorization: `Bearer ${adminToken}`
        }
      });
  ...
  const getCategoryImages = (category) => {
    ...
    if (Array.isArray(category.pic)) {
      ...
      try {
        const parsed = JSON.parse(category.pic);
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  setShowModal(false);
                  handleDeleteClick(selectedCategory.id);
                }}
