...
export default function ViewAllProducts() {
  ...
  // ✅ FIX: Ref to track setTimeout IDs for cleanup
  ...
  const fetchProducts = async (isRetry = false) => {
    try {
      ...
      // Mechanical seal attributes are fetched once categories are available
    } catch (err) {
      ...
      // ✅ FIX: Handle network errors with retry logic and cleanup
      if (err.code === 'ECONNABORTED' || err.code === 'ERR_NETWORK' || !err.response) {
        if (retryCount < 3) {
          console.log(`🔄 Retrying... Attempt ${retryCount + 1}/3`);
          setRetryCount(prev => prev + 1);
          // ✅ FIX: Store timeout ID for cleanup
          retryTimeoutRef.current = setTimeout(() => fetchProducts(true), 2000 * (retryCount + 1));
          return;
        }
      }
  ...
  const fetchCategories = async () => {
    try {
      ...
      const res = await axios.get(buildApiUrl('/api/categories'), {
        headers: {
          Authorization: `Bearer ${adminToken}`
        },
        timeout: 10000
  ...
  const fetchMechanicalSealAttributes = async (productsToCheck) => {
    try {
      const adminToken = localStorage.getItem('adminToken');
      
      if (!adminToken || !Array.isArray(productsToCheck) || productsToCheck.length === 0) {
        setMechanicalSealAttributes({});
        return;
      }
      ...
      const responses = await Promise.all(
        productsToCheck.map(async (product) => {
          try {
            const res = await axios.get(buildApiUrl(`/api/mechanical-seals/product/${product.id}`), {
              headers: {
                Authorization: `Bearer ${adminToken}`
              },
              timeout: 5000
            ...
            if (res.data && res.data.length > 0) {
              return { productId: product.id, data: res.data };
            }
          } catch (err) {
      ...
      responses.forEach((result) => {
        if (result?.data) {
          attributes[result.productId] = result.data;
        }
      });
  ...
  useEffect(() => {
    ...
    const mechanicalCategoryIds = categories
      .filter(cat =>
        cat.special_category === true ||
        normalizeCategoryName(cat.name) === MECHANICAL_SEAL_CATEGORY_SLUG
      )
      .map(cat => cat.id);

    if (mechanicalCategoryIds.length === 0) {
      setMechanicalSealAttributes({});
      return;
    }

    const mechanicalProducts = products.filter(product =>
      mechanicalCategoryIds.includes(product.category_id)
    );
    ...
    fetchMechanicalSealAttributes(mechanicalProducts);
  }, [products, categories]);
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const adminToken = localStorage.getItem('adminToken');
      await axios.delete(buildApiUrl(`/api/products/${productToDelete}`), {
        headers: {
          Authorization: `Bearer ${adminToken}`
        }
      });
  ...
  const getProductImages = (product) => {
    ...
    if (Array.isArray(product.pic)) {
      ...
      try {
        const parsed = JSON.parse(product.pic);
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              ...
              className="pl-10 input-search admin-input"
            />
          </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedProduct.product_Code && (
                      <div>
                        ...
                        <p className="font-semibold text-red-600">{selectedProduct.product_Code}</p>
                      </div>
                    )}
                    <div>
