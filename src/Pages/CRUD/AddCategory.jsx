  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateForm()) {
      toast.error('Please fix the errors before submitting', {
        toastId: 'add-category-validation-error'
      });
      return;
    }
    ...
    try {
      ...
      if (!adminToken) {
        toast.error('Authentication required. Please login again.', {
          toastId: 'add-category-auth-error'
        });
      ...
      if (error.code === 'NETWORK_ERROR') {
        toast.error('Network error. Please check your connection.', {
          toastId: 'add-category-network-error'
        });
      } else if (error.response?.status === 413) {
        toast.error('File too large. Please reduce image size.', {
          toastId: 'add-category-file-size-error'
        });
      } else if (error.response?.data?.message) {
        toast.error(error.response.data.message, {
          toastId: 'add-category-api-error'
        });
      } else {
        toast.error('Failed to create category. Please try again.', {
          toastId: 'add-category-unknown-error'
        });
      }
    } finally {
      setLoading(false);
