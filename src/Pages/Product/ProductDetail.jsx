...
const ProductDetail = () => {
  ...
  const checkIfMechanicalSeal = useCallback((categoryId) => {
    ...
    return (
      ...
      normalizeCategoryName(category?.name) === MECHANICAL_SEAL_CATEGORY_SLUG
    );
  }, [categories]);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const response = await axios.get(buildApiUrl(`/api/products/${id}`));
        ...
        toast.error("Failed to load product details", {
          toastId: 'product-detail-load-error'
        });
        navigate('/product');
      } finally {
        setLoading(false);
  ...
  const handleAddToCart = async () => {
    if (!isAuthenticated) {
      toast.error("Please login to add items to cart", {
        toastId: 'product-detail-login-required'
      });
      navigate('/login');
      return;
    }
    ...
    if (!selectedSize && product?.sizes?.length > 0) {
      toast.warning("Please select a size", {
        toastId: 'product-detail-size-required'
      });
      return;
    }
    ...
    try {
      ...
      toast.success(`✅ Added ${quantity} ${product.name} to cart!`, {
        toastId: 'product-detail-add-to-cart-success'
      });
      ...
      if (error.response?.status === 401) {
        ...
        toast.error(error.response?.data?.error || "Failed to add item to cart", {
          toastId: 'product-detail-add-to-cart-error'
        });
      }
    } finally {
  ...
  const handleBuyNow = () => {
    if (!selectedSize && product?.sizes?.length > 0) {
      toast.warning("Please select a size", {
        toastId: 'product-detail-buy-now-size-required'
      });
      return;
    }
    ...
    toast.success("Redirecting to checkout...", {
      toastId: 'product-detail-buy-now-success'
    });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-4">
              {/* First Row - Labels */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 sm:col-span-2">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-6 sm:col-span-2">
                <div className="flex-1">
                  <h1
                    className="font-bold text-white leading-tight text-xl sm:text-2xl"
                    style={{ fontSize: product.name.length > 26 ? "1.25rem" : undefined }}
                  >
                    Model: {product.name}
                  </h1>
                  {product.product_Code && (
                    <p className="text-base sm:text-lg text-red-400 mt-1 font-medium">
                ...
                {product.sizes && product.sizes.length > 0 && (
                  <select
                    ...
                    className="w-full sm:w-32 p-2 bg-gray-800 border border-gray-600 rounded-md text-white text-sm focus:outline-none focus:border-red-500"
                  >
